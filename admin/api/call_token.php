<?php
/**
 * Admin API - Call Token
 * Mark a token as called and move to counter
 */
...
// Log API call
error_log("Call Token API - Admin ID: {$_SESSION['admin_id']}, Request: " . print_r($_REQUEST, true));
...
if ($token['status'] !== 'Active') {
    http_response_code(400);
    $response['message'] = 'Token is not in active status';
    echo json_encode($response);
    exit;
}

// Update token status to Called
$update_stmt = $db->prepare("UPDATE tokens SET status = 'Called', called_at = NOW() WHERE id = ?");
...
if (!$update_stmt->execute()) {
    http_response_code(500);
    $response['message'] = 'Failed to call token: ' . $update_stmt->error;
    header('Content-Type: application/json');
    echo json_encode($response);
    exit;
}
...
// Log the action
$log_stmt = $db->prepare("INSERT INTO admin_logs (admin_id, action, token_id, details, created_at) VALUES (?, 'CALL_TOKEN', ?, ?, NOW())");
$details = "Called token: {$token['token_number']}";
$log_stmt->bind_param("iss", $_SESSION['admin_id'], $token_id, $details);
$log_stmt->execute();

$response['success'] = true;
$response['message'] = 'Token called successfully';
