...
if (!in_array($token['status'], ['Active', 'Called'])) {
    http_response_code(400);
    $response['message'] = 'Token cannot be completed from current status';
    echo json_encode($response);
    exit;
}

// Update token status to Completed
$update_stmt = $db->prepare("UPDATE tokens SET status = 'Completed', completed_at = NOW() WHERE id = ?");
...
if (!$update_stmt->execute()) {
    http_response_code(500);
    $response['message'] = 'Failed to complete token: ' . $update_stmt->error;
    header('Content-Type: application/json');
    echo json_encode($response);
    exit;
}
...
// Log the action
$log_stmt = $db->prepare("INSERT INTO admin_logs (admin_id, action, token_id, details, created_at) VALUES (?, 'COMPLETE_TOKEN', ?, ?, NOW())");
$details = "Completed token: {$token['token_number']}";
$log_stmt->bind_param("iss", $_SESSION['admin_id'], $token_id, $details);
$log_stmt->execute();

$response['success'] = true;
$response['message'] = 'Token completed successfully';
